...
def delete_cart_item(request, pk):
    customer = Customer.objects.get(email=request.session["email"])
    product = Product.objects.get(pk=pk)
    item = Cart.objects.get(customer=customer, product = product)
    item.delete()
    return redirect("product_cart_display")
...
#     # FIRST: Remove corrupted rows BEFORE querying
#     try:
#         Blog.objects.filter(id='').delete()
#     except:
#         pass
    
#     try:
#         Blog.objects.filter(id__isnull=True).delete()
#     except:
#         pass

#     # NOW: safely fetch blogs
#     blogs = Blog.objects.all().exclude(id='').exclude(id__isnull=True)

#     # Debug output
#     print("\nCLEAN BLOG IDS =>", [b.id for b in blogs], "\n")
