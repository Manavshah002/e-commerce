    ...
    if request.method == "POST":
        ...
        if title and price and desc:
            product = Product(
                ...
                quantity=quantity,
                desc=desc,
                image=image if image else "images/sample.jpg",
                trader=seller,
                in_stock=in_stock
            )
            product.save()
            msg_s = "Product added successfully!"
        else:
            msg_d = "Please fill all required fields."

    return render(request, 'seller-add_product.html', {'msg_s': msg_s, 'msg_d': msg_d})
